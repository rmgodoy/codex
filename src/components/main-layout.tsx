...
export default function MainLayout({
    ...
    reader.onload = async (e) => {
      try {
        ...
        toast({
          ...
          description:
            "Data has been overwritten. The application will now reload.",
        });
  ...
  const handleExport = async () => {
    ...
    try {
      ...
      toast({
        ...
        description: `"${worldName || worldSlug}" data has been downloaded.`,
      });
    } catch (error) {
  ...
  const finalPageTitle = useMemo(() => {
    ...
    if (pageKey) {
      ...
      if (pageKeyLower.startsWith("random/encounter-tables"))
        pageName = "Encounter Tables";
      else if (pageKeyLower.startsWith("random/treasures"))
        pageName = "Treasures";
      ...
      else if (pageKeyLower.startsWith("calendar-models")) pageName = "Calendar Models";
    ...
    <>
      ...
      {otherLinks.map((link) => (
        <a href={link.href} key={link.href}>
          <Button
            variant={
              hash.endsWith(link.href.split("/").pop()!) ? "secondary" : "ghost"
            }
          >
            {link.label}
          </Button>
