...
import { getAllEncounterTables } from '@/lib/idb';
...
interface EncounterTableListPanelProps {
...
export default function EncounterTableListPanel({ 
  onSelectTable, 
  onNewTable, 
  selectedTableId, 
  dataVersion,
  ...
  useEffect(() => {
    const fetchAllData = async () => {
      setIsLoading(true);
      try {
        setTables(await getAllEncounterTables());
      } catch (error) {
        console.error("Error fetching encounter tables:", error);
        toast({ variant: "destructive", title: "Error", description: "Could not fetch encounter tables from database." });
      } finally {
        setIsLoading(false);
      }
    };
  ...
  const filteredAndSortedTables = useMemo(() => {
    let filtered = tables.filter(table => {
      ...
      if (filters.minTR && !isNaN(parseInt(filters.minTR))) {
        matchesTR = matchesTR && table.totalTR >= parseInt(filters.minTR, 10);
      }
      if (filters.maxTR && !isNaN(parseInt(filters.maxTR))) {
        matchesTR = matchesTR && table.totalTR <= parseInt(filters.maxTR, 10);
      }
    ...
    <div className="flex flex-col h-full">
      <div className="p-4 space-y-4">
        <Button onClick={onNewTable} className="w-full">
          <PlusCircle /> New Encounter Table
        </Button>
        ...
        <div className="space-y-2">
            ...
            <div className="relative">
                ...
                <Input placeholder="Tags..." value={filters.tagFilter} onChange={e => setFilters.setTagFilter(e.target.value)} className="pl-9"/>
            </div>
            ...
            <ul className="space-y-1">
              {filteredAndSortedTables.map(table => (
                <li key={table.id}>
                  ...
                  >
                    {table.name} <span className="text-xs opacity-70">(TR {table.totalTR})</span>
                  </button>
                </li>
              ))}
            </ul>
