...
export function CalendarEventDialog({
  ...
  useEffect(() => {
    Promise.all([
      ...
      getAllMaps(),
      getAllNpcs(),
    ]).then(([creatures, factions, maps, npcs]) => {
    ...
    if (isOpen) {
      if (event) {
        ...
        if (event.location && allMaps.length > 0) {
          const mapName =
            allMaps.find((m) => m.id === event.location!.mapId)?.name ||
            "Unknown Map";
  ...
  const onSubmit = async (data: EventFormData) => {
    ...
    const eventToSave: NewCalendarEvent = {
      ...
      location: selectedLocation
        ? { mapId: selectedLocation.mapId, hex: selectedLocation.hex }
        : undefined,
    };
                      ...
                      {calendarModel ? (
                        ...
                        >
                          ...
                          >
                            <span>
                              {field.value &&
                              calendarModel.months[field.value.monthIndex]
                                ? `${
                                    calendarModel.months[field.value.monthIndex]
                                      .name
                                  } ${field.value.day}, ${field.value.year}`
                                : "Pick a date"}
                            </span>
                        ...
                        <Popover>
                          ...
                          <PopoverContent className="w-auto p-0" align="start">
                            <Calendar
                              mode="single"
                              selected={customDateToDate(field.value)}
                              onSelect={(d) =>
                                d && field.onChange(dateToCustomDate(d))
                              }
                              defaultMonth={customDateToDate(field.value)}
                              disabled={(date) => date < getYearOne()}
                              initialFocus
                            />
                          </PopoverContent>
                        </Popover>
                        ...
                        <Popover>
                          ...
                          <PopoverContent className="w-auto p-0" align="start">
                            <Calendar
                              ...
                              defaultMonth={customDateToDate(
                                field.value || watchedStartDate
                              )}
                              disabled={(date) =>
                                date <
                                (customDateToDate(watchedStartDate) ||
                                  getYearOne())
                              }
                              initialFocus
                            />
                ...
                {selectedParty ? (
                  ...
                  <div className="grid grid-cols-3 gap-2 mt-2">
                    <CalendarPartySelectionDialog
                      items={factions}
                      onSelectItem={(item) =>
                        setSelectedParty({ ...item, type: "faction" })
                      }
                      trigger={
                        <Button
                          type="button"
                    ...
                    <CalendarPartySelectionDialog
                      items={creatures}
                      onSelectItem={(item) =>
                        setSelectedParty({ ...item, type: "creature" })
                      }
                      trigger={
                        <Button
                          type="button"
                    ...
                    <CalendarPartySelectionDialog
                      items={npcs}
                      onSelectItem={(item) =>
                        setSelectedParty({ ...item, type: "npc" })
                      }
                      trigger={
                        <Button
                          type="button"
                ...
                {selectedLocation ? (
                  <div className="flex items-center justify-between p-2 mt-2 border rounded-md">
                    <p className="font-semibold text-sm">
                      ...
                      Q:{selectedLocation.hex.q}, R:{selectedLocation.hex.r}
                    </p>
                    <Button
                      type="button"
                      variant="ghost"
                  ...
                  <LocationPickerDialog
                    onLocationSelect={({ mapId, hex }) => {
                      const mapName =
                        allMaps.find((m) => m.id === mapId)?.name ||
                        "Unknown Map";
