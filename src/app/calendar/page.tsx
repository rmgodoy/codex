...
import {
  ...
  deleteCalendarEvent,
} from "@/lib/idb/calendarEvents";
import {
  getAllCalendars,
  addCalendar,
  updateCalendar,
  deleteCalendarAndEvents,
...
import type {
  ...
  NewCalendar,
  CustomCalendar as CustomCalendarType,
  CustomDate,
} from "@/lib/types";
import {
  Select,
...
function CalendarManagementDialog({
  ...
  customCalendars: CustomCalendarType[];
  onCalendarsUpdate: (newCalendarId?: string) => void;
}) {
  ...
  const handleAddCalendar = async () => {
    ...
    try {
      const calendarData: NewCalendar = {
        ...
        modelId:
          newCalendarModelId === "traditional" ? undefined : newCalendarModelId,
      };
      const newId = await addCalendar(calendarData);
      toast({
        title: "Calendar Created",
        description: `"${newCalendarName}" has been added.`,
      });
      setNewCalendarName("");
  ...
  const handleRenameCalendar = async () => {
    ...
    try {
      ...
      toast({
        variant: "destructive",
        title: "Error",
        description: "Failed to rename calendar.",
      });
    }
  };
  ...
  const handleDeleteCalendar = async (calendarId: string) => {
    try {
      ...
      toast({
        variant: "destructive",
        title: "Error",
        description: "Failed to delete calendar and its events.",
      });
    }
  };
    ...
    <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
      ...
      <DialogContent>
        <DialogHeader>
          ...
          <DialogDescription>
            Add, rename, or delete your calendars.
          </DialogDescription>
        </DialogHeader>
        <div className="space-y-4 py-4">
          <div className="flex flex-col gap-2 p-3 border rounded-md">
            ...
            />
            <Select
              value={newCalendarModelId}
              onValueChange={setNewCalendarModelId}
            >
              <SelectTrigger>
            ...
            {calendars.map((cal) => (
              ...
              >
                {editingId === cal.id ? (
                  ...
                  <>
                    <span className="flex-1 truncate" title={cal.name}>
                      {cal.name}
                    </span>
                      ...
                      <AlertDialog>
                        ...
                        <AlertDialogContent>
                          <AlertDialogHeader>
                            ...
                            <AlertDialogDescription>
                              This will permanently delete the "{cal.name}"
                              calendar and all of its events.
                            </AlertDialogDescription>
                          </AlertDialogHeader>
                          <AlertDialogFooter>
...
export default function CalendarPage({ selectedId, selectedDate }: CalendarPageProps) {
  ...
  const activeCalendarModel = useMemo(() => {
    ...
    return (
      customCalendars.find((cm) => cm.id === activeCalendar.modelId) || null
    );
  ...
  const fetchCalendarsAndEvents = async (calendarId: string | null) => {
    try {
      ...
      if (
        !finalCalendarId ||
        !allCalendars.some((c) => c.id === finalCalendarId)
      ) {
        finalCalendarId = allCalendars[0]?.id || null;
      }
      setCalendars(allCalendars);

      if (selectedCalendarId === finalCalendarId && allCalendars.length > 0)
        return;
    ...
    const loadEventsAndSetDate = async () => {
      ...
      const currentModel =
        customCalendars.find(
          (cm) =>
            cm.id ===
            calendars.find((c) => c.id === selectedCalendarId)?.modelId
        ) || null;
      ...
      if (allEvents.length > 0) {
        const sortedEvents = [...allEvents].sort(
          (a, b) =>
            customDateToDate(a.startDate).getTime() -
            customDateToDate(b.startDate).getTime()
        );
        ...
        <div className="h-full flex flex-col items-center justify-center p-8 text-center bg-background/50">
          <Card className="max-w-md">
            <CardHeader>
              ...
              <CardDescription>
                Create your first calendar to start tracking important dates and
                events in your world.
              </CardDescription>
            </CardHeader>
            <CardContent>{calendarManager}</CardContent>
          </Card>
        </div>
      </MainLayout>
    );
            ...
            <SidebarInset className="flex-1 flex flex-col overflow-hidden bg-background/50 p-4">
              {isCustomCalendar && activeCalendarModel ? (
                ...
                <Calendar
                  ...
                  value={
                    selectedCustomDate
                      ? customDateToDate(selectedCustomDate)
                      : undefined
                  }
                  ...
                  tileContent={({ date, view }) => {
                    if (view === "month") {
                      const hasEvent = eventDays.some((eventDay) =>
                        isSameDay(customDateToDate(eventDay), date)
                      );
                      return hasEvent ? (
