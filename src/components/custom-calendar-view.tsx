...
interface CustomCalendarViewProps {
  ...
  initialDate?: CustomDate | string | null;
...
export function CustomCalendarView({
  ...
  const getInitialCustomDate = (): CustomDate | null => {
    if (typeof initialDate == "string") {
      initialDate = dateToCustomDate(
        moment(initialDate).utc().endOf("day").toDate()
      );
    }
    if (initialDate) {
  ...
  const [currentDate, setCurrentDate] = useState<CustomDate | null>(
    getInitialCustomDate()
  );
    ...
    events.forEach((event) => {
      ...
      while (true) {
        ...
        if (
          ...
          currentMonthIndex === end.monthIndex &&
          currentDay === end.day
        ) {
          break;
        }
  ...
  const handlePrev = () => {
    if (viewMode === "day") {
      ...
      setCurrentDate((prev) =>
        prev ? { ...prev, year: prev.year - 1 } : null
      );
    } else {
      // year
  ...
  const handleYearSubmit = (e: React.FormEvent) => {
    ...
    if (!isNaN(newYear)) {
      setCurrentDate((prev) =>
        prev
          ? { ...prev, year: newYear }
          : { year: newYear, monthIndex: 0, day: 1 }
      );
      setViewMode("month");
  ...
  const handleMonthSelect = (monthIndex: number) => {
    setCurrentDate((prev) =>
      prev
        ? { ...prev, monthIndex, day: 1 }
        : { year: new Date().getFullYear(), monthIndex, day: 1 }
    );
    setViewMode("day");
  };
  ...
  const handleYearSelect = (year: number) => {
    setCurrentDate((prev) =>
      prev ? { ...prev, year } : { year, monthIndex: 0, day: 1 }
    );
    setViewMode("month");
  };
      ...
      >
        ...
        >
          {calendar.weekdays.map((day) => (
            <div
              key={day}
              className="text-center font-semibold text-muted-foreground p-2 text-sm"
            >
              {day}
            </div>
          ))}
